...
Dataset_Info <- readRDS("C:/Users/m/Documents/shiny-server/databases/microarray_data_infosheet_R.RDS")

#load and name GEO microarray and RNA-Seq datasets
for (i in Dataset_Info$Unique_ID) {assign(i, readRDS(paste0("C:/Users/m/Documents/shiny-server/databases/microarray_results/", i, ".RDS")))} 
...
tfbs <- readRDS("C:/Users/m/Documents/shiny-server/databases/tfbs_for_app.RDS") #TFBS data from ENCODE - matched to gene ids using bedtools
snp <- readRDS("C:/Users/m/Documents/shiny-server/databases/grasp_output_for_app.RDS") #SNP data from GRASP - matched to gene ids using bedtools
snp_eve <- readRDS("C:/Users/m/Documents/shiny-server/databases/eve_data_realgar.RDS") #SNP data from EVE - was already in hg19 - matched to gene ids using bedtools 
snp_gabriel <- readRDS("C:/Users/m/Documents/shiny-server/databases/gabriel_data_realgar.RDS") #SNP data from GABRIEL - lifted over from hg17 to hg19 - matched to gene ids using bedtools 
snp_fer <- readRDS("C:/Users/m/Documents/shiny-server/databases/allerg_GWAS_data_realgar.RDS")
snp_TAGC <- readRDS("C:/Users/m/Documents/shiny-server/databases/TAGC_data_realgar.RDS")
gene_locations <- fread("C:/Users/m/Documents/shiny-server/databases/gene_positions.txt", header = TRUE, stringsAsFactors = FALSE) #gene location & transcript data from GENCODE
chrom_bands <- readRDS("C:/Users/m/Documents/shiny-server/databases/chrom_bands.RDS") #chromosome band info for ideogram - makes ideogram load 25 seconds faster
...
server <- shinyServer(function(input, output, session) {
    ...
    output$color_scale1 <- output$color_scale2 <- renderImage({ #need two separate output names - else it fails (can't output same thing twice?)
        return(list(
            src = "C:/Users/m/Documents/shiny-server/databases/www/color_scale_vertical.png",
            height=550,
            width=59,
            filetype = "image/png",
